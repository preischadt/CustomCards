...
function c420000059.initial_effect(c)
    --link summon
    Link.AddProcedure(c, nil, 2, 2, c420000059.lcheck)
    c:EnableReviveLimit()
...
function c420000059.hspfilter(c, e, tp)
    return c:IsCode(28297833) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c420000059.hsptg(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk == 0 then
        return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and
            Duel.IsExistingMatchingCard(c420000059.hspfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
    end
...
function c420000059.spfilter(c, e, tp)
    return c:IsRace(RACE_ZOMBIE) and c:IsAttribute(ATTRIBUTE_DARK) and (c:IsType(TYPE_SYNCHRO) or c:IsType(TYPE_XYZ)) and
        c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c420000059.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk == 0 then
        return Duel.IsExistingMatchingCard(c420000059.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
    end
