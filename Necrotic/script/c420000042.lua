...
function c420000042.initial_effect(c)
	...
	c:EnableReviveLimit()
	--materials
	aux.AddFusionProcCodeFun(c,28297833,c420000042.ffilter,1,true,true)
	--limit
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetCode(EFFECT_SPSUMMON_CONDITION)
	e1:SetValue(c420000042.splimit)
	c:RegisterEffect(e1)
	--special summon rule
	...
	e2:SetValue(1)
	e2:SetCondition(c420000042.sprcon)
	e2:SetOperation(c420000042.sprop)
	e2:SetCountLimit(1, 420000042)
	c:RegisterEffect(e2)
	...
	-- e3:SetValue(c420000042.synlimit)
	-- c:RegisterEffect(e3)
end
...
function c420000042.spfilter1(c,tp)
	return (c:IsCode(28297833) or c420000042.ffilter(c)) and c:IsAbleToRemoveAsCost() and c:IsCanBeFusionMaterial(nil,true)
		and Duel.IsExistingMatchingCard(c420000042.spfilter2,tp,LOCATION_ONFIELD,0,1,c,c)
end
function c420000042.spfilter2(c, c1)
	return ((c1:IsCode(28297833) and c420000042.ffilter(c)) or (c:IsCode(28297833) and c420000042.ffilter(c1))) and c:IsAbleToRemoveAsCost() and c:IsCanBeFusionMaterial(nil,true)
end
function c420000042.sprcon(e,c)
	if c==nil then return true end 
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-2
		and Duel.IsExistingMatchingCard(c420000042.spfilter1,tp,LOCATION_ONFIELD,0,1,nil,tp)
end
function c420000042.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	...
	local g2=Duel.SelectMatchingCard(tp,c420000042.spfilter2,tp,LOCATION_ONFIELD,0,1,1,g1:GetFirst(),g1:GetFirst())
	g1:Merge(g2)
...
function c420000042.filter(c,e,tp)
	return c:IsFaceup() and (c:IsRace(RACE_ZOMBIE) or c:IsAttribute(ATTRIBUTE_DARK)) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function c420000042.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REMOVED) and c420000042.filter(chkc,e,tp) end
	if chk==0 then return true end --Duel.GetLocationCount(tp,LOCATION_MZONE)>0 end
