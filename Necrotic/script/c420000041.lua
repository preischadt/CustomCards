...
function c420000041.spfilter1(c, tp)
	return c:IsCode(28297833) and c:IsAbleToRemoveAsCost() and c:IsCanBeFusionMaterial(nil, true) and
		Duel.IsExistingMatchingCard(c420000041.spfilter2, tp, LOCATION_MZONE, 0, 1, c)
end
function c420000041.spfilter2(c)
	return c:IsType(TYPE_SYNCHRO) and (c:IsRace(RACE_ZOMBIE) or c:IsAttribute(ATTRIBUTE_DARK)) and c:IsAbleToRemoveAsCost() and
...
function c420000041.sprcon(e, c)
	...
	return Duel.GetLocationCount(tp, LOCATION_MZONE) > -2 and
		Duel.IsExistingMatchingCard(c420000041.spfilter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
end
